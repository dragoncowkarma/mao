...
- **Real `mao swarm` runs have their own credential-aligned preflight.** Before Git sync, worktree
  ...
  capability probe. Custom `core.sshCommand`, `GIT_SSH_COMMAND`, and `GIT_SSH` overrides are rejected
  for SSH origins because the system `ssh -G` result would not prove their actual target. Effective
  ...
  `git push ... origin <branch>` rather than a remote-less push. A revision is dispatched only for
  the shell-safe `worker/<issue>-<slug>` branch format generated by Swarm; never interpolate an
  arbitrary GitHub head ref into an executable prompt. A
